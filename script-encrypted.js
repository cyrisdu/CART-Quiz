...
function initializeApp() {
    ...
    if (users.length === 0) {
        ...
        cryptoStorage.setUsers(defaultUsers);
    }
    ...
    if (wrongQuestions.length === 0) {
        ...
        wrongAnswersContainer.innerHTML = `
            <div class="errors-section">
                <h3 class="errors-title">📝 错题回顾 (${wrongQuestions.length}题)</h3>
                <div class="errors-list">
                    ${wrongQuestions.map((q, index) => `
                        <div class="error-item">
                            <div class="question-text">${q.description}</div>
                            <div class="answer-text">✅ ${q.correctAnswerText || q.options[q.correctAnswer - 1]}</div>
                        </div>
                    `).join('')}
                </div>
            </div>
...
function clearUserData() {
    cryptoStorage.clearAllData();
    console.log('✅ 加密用户数据已清除，页面将重新加载');
    location.reload();
}
